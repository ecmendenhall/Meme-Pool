...
def get_all_followers(client, base_hostname):
...
def get_random_follower(client, base_hostname):
    """Gets a random follower of the given user. Returns a dict."""
    userslist = get_all_followers(client, base_hostname)
...
def get_tagged_image_from_follower(client, follower):
    ...
    if not follower:
        ...
        if all(post['tags'] == 0 for post in follower_posts_with_tags):
            new_follower = get_random_follower(client, 'meme-pool')
...
def reblog_to_memepool(client, reblog_target):
    ...
    if not reblog_target:
        ...
        logging.info("Reblog target key: %s", reblog_target_key)
...
def sort_memepool_by_fitness(client):
...
def mate_posts(client, sorted_list):
    ...
    allele_photos = flickr.photos_search(tags=image_tags, tag_mode='all', license='1,2,3,4,5,6,7', sort='interestingness-desc')[14:-1]
    
    if len(allele_photos) == 0:
        allele_photos = flickr.photos_search(tags=image_tags, license='1,2,3,4,5,6,7', sort='interestingness-desc')[14:-1]
    
    #Sort returned photos by interestingness and choose more interesting photos more often
    number_of_interesting_photos = len(allele_photos) - 1
    chosen_photo = int(random.random() * random.random() * number_of_interesting_photos)
    logging.info("allele_photos: %s", allele_photos)
    logging.info("len(allele_photos: %s", len(allele_photos))
    logging.info("chosen_photo: %s", chosen_photo)
    most_interesting_photo = allele_photos[chosen_photo]
    ...
    photo_license_id = most_interesting_photo.license
    photo_license = ""
...
def save_generation_stats(client, sorted_memepool):
    ...
    all_followers = get_all_followers(client, 'meme-pool')
...
def main():
    ...
    set_up_flickr(flickr_api_key)
    sorted_memepool = sort_memepool_by_fitness(client)
    mate_posts(client, sorted_memepool)
    
    follower1 = get_random_follower(client, 'meme-pool')
    follower1_reblog_target = get_tagged_image_from_follower(client, follower1)
    reblog_to_memepool(client, follower1_reblog_target)

    mate_posts(client, sorted_memepool)

    follower2 = get_random_follower(client, 'meme-pool')
    follower2_reblog_target = get_tagged_image_from_follower(client, follower2)
    reblog_to_memepool(client, follower2_reblog_target)

    mate_posts(client, sorted_memepool)

    save_generation_stats(client, sorted_memepool)
